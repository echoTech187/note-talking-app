<div class=" w-full h-full flex-col items-center justify-center gap-16 hidden lg:flex mb-32">
    ...
    <div
        ...
        <span class="text-4xl font-bold text-white">Buat Akun Baru</span>
        <p class="mt-2  text-white ">Buat akun baru untuk melanjutkan Note Talking</p>
    </div>
</div>
<div
    ...
    <h1 class="text-2xl font-bold text-[#444444] dark:text-[#efefef]">Sign Up</h1>
    @if (session()->has('message'))
        <div class="mt-6 text-sm bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded">
            {{ session()->get('message') }}
        </div>
    @endif
    <form action="{{ route('register.post') }}" name="signinFrm" id="signinFrm" method="POST" class="mt-6 w-full">
        @csrf
        <x-frontend.input name="name" type="text" placeholder="Nama Lengkap" label="Nama Lengkap" />
        <x-frontend.input name="email" type="email" placeholder="Email" label="Email" />
        <x-frontend.input name="password" type="password" placeholder="Password" label="Password" />
        <x-frontend.input name="confirm_password" type="password" placeholder="Konfirmasi Password"
    ...
    <div class="flex items-center justify-center mt-6">
        <span class="text-sm text-[#444444] dark:text-[#efefef]">Sudah Punya Akun?</span>
        <a href="{{ route('login') }}" class="text-sm text-blue-600 hover:underline dark:text-[#efefef] ml-1">Masuk</a>
    </div>
</div>
